...
def fillWithAttibutesValues(attributes_list, storage_variable, storage_variable_new_len, annotation_quantity):
    ...
    :param storage_variable: list - Temp variable (already containing resaerched extent informations)
...
words_to_find = [x.rstrip() for x in open('OUTPUT/00_Extent-to-search.txt', 'r', encoding='UTF-8')]
...
for x in range(0,len(words_to_find)):
    words_to_find.append(words_to_find[x] + '.')
    words_to_find.append(words_to_find[x] + ',')

for x in range(0,len(words_to_find)):
    if '.' not in words_to_find[x] and ',' not in words_to_find[x]:
        words_to_find[x] += ' '

if ' ' in words_to_find:
    del words_to_find[words_to_find.index(' ')]
...
lignes = []
if csv == True:
    csv_name = 'Searched_results/ALL-SEARCHED-EXTENT.csv'
    with open(csv_name, 'w', encoding='utf-8') as csv_file:
        csv_file.write('Word\tfile\tSentence_number\tWord_start_position\tWord_end_position\tcontexte_gauche\tcontexte_droit\tSentence\tEVENT_eventType\tEVENT_polarity\tEVENT_contextualModality\tEVENT_docTimeRel\tTIMEX3_timex3Class\tTIMEX3_value\tCLINENTITY_EntityID\tACTOR_role\tEtat de l\'annotation\tCode\tCommentaire\n')
